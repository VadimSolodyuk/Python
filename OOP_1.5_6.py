...
class Graph:
    ...
    def __get_data_as_str(self):
        return ' '.join(map(str, self.data))
    
    def __check_is_show(self):
        if not self.is_show:
    ...
    def show_table(self):
        if self.__check_is_show():
            print(self.__get_data_as_str())
        
    def show_graph(self):
        if self.__check_is_show():
            print(f'Графическое отображение данных: {self.__get_data_as_str()}')
        
    def show_bar(self):
        if self.__check_is_show:
            print(f'Столбчатая диаграмма: {self.__get_data_as_str()}')
